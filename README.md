...
2. 若要警示，到「**警示名單**」選擇類型（高風險走失 / 失智長者 / 黑名單）→ 點「建立名單」啟用
...
### 警示名單 tab
- 固定三種類型：**高風險走失**（critical 音效）/ **失智長者**（warn 音效）/ **黑名單**（critical 音效）
- 每種名單需先點「建立名單」啟用；每個名單顯示成員清單，加入新成員用下拉選人員（不用打 ID）
- 其他身份分類（家屬 / 員工 / 志工 / 重要訪客 / 訪客）請在「人員」tab 編輯「身份」欄位，不觸發警示音效
